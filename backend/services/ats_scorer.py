...
def detect_location_info(text: str, nlp: spacy.Language) -> Dict:
    ...
    if has_address:
        recommendations.append('Remove full street addresses — ATS systems do not need them and they are a privacy risk.')
    if has_zip:
...
def _calculate_semantic_similarity(skill: str, text_vec: np.ndarray, embedder: SentenceTransformer) -> float:
    """Compute cosine similarity between a skill string and a pre-computed text embedding."""
    if not skill:
        return 0.0
    try:
        skill_vec = embedder.encode(skill, convert_to_tensor=False)
        similarity = np.dot(skill_vec, text_vec) / (
            np.linalg.norm(skill_vec) * np.linalg.norm(text_vec) + 1e-10
        )
...
def _skill_matches_text(skill: str, raw_text: str, text_vec: np.ndarray,
                        embedder: SentenceTransformer, threshold: float) -> Tuple[bool, float]:
    """Fast substring check first; falls back to semantic similarity."""
    if skill.lower() in raw_text.lower():
        return True, 1.0
    sim = _calculate_semantic_similarity(skill, text_vec, embedder)
...
def validate_skills_with_projects(
    ...
    # Pre-compute the experience embedding ONCE outside the skill loop
    experience_vec: Optional[np.ndarray] = None
    if experience_text:
        try:
            experience_vec = embedder.encode(experience_text, convert_to_tensor=False)
        except Exception:
            experience_vec = None

    # Pre-compute per-project text and embeddings
    project_data: List[Tuple[str, str, Optional[np.ndarray]]] = []
    for project in projects:
        p_text = f"{project.get('title', '')} {project.get('description', '')}"
        try:
            p_vec = embedder.encode(p_text, convert_to_tensor=False) if p_text.strip() else None
        except Exception:
            p_vec = None
        project_data.append((project.get('title', 'Untitled Project'), p_text, p_vec))
    ...
    for skill in skills:
        ...
        # Check projects
        for title, p_text, p_vec in project_data:
            if p_vec is None:
                continue
            if skill.lower() in p_text.lower():
                matched, sim = True, 1.0
            else:
                sim     = _calculate_semantic_similarity(skill, p_vec, embedder)
                matched = sim >= threshold
            max_similarity = max(max_similarity, sim)
            if matched:
                matching_projects.append(title)

        # Check experience section (pre-computed vector)
        if experience_vec is not None:
            if skill.lower() in experience_text.lower():
                matched, sim = True, 1.0
            else:
                sim     = _calculate_semantic_similarity(skill, experience_vec, embedder)
                matched = sim >= threshold
            max_similarity = max(max_similarity, sim)
...
def _calc_ats_compatibility_score(
    ...
    edu_desc_len = sum(
        len((e.get('degree') or '') + (e.get('institution') or ''))
